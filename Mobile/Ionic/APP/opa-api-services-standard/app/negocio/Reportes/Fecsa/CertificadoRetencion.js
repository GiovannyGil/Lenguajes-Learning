...
function generate(data, dataR, data1) {
  const promise = new RSVP.Promise((resolve, reject) => {
    const header = (rpt) => {
      rpt.image('./imagen/Fecsa/LogoEntidad.PNG');
      rpt.newLine(2);
      ...
      let empieza = 0;
      const termina = data1.length;

      while (empieza >= 0 && empieza < termina) {
        rpt.band([
          { data: data1[empieza].nombre, width: 410 },
          {
            data: data1[empieza].valor,
            width: 80,
            align: 'right',
          },
        ]);
      ...
      if (fs.existsSync('./imagen/Fecsa/LogoCpio.PNG')) {
        const imgLoc = './imagen/Fecsa/LogoCpio.PNG';
        rpt.image(imgLoc);
      } else {
      ...
      rpt.print(`PBX: ${dataR.telefono}`, {
        align: 'center',
      });
      rpt.print(`E-mail: ${dataR.email}`, {
        align: 'center',
      });
    };
