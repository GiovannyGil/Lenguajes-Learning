...
@Component({
  selector: 'app-seguros-vehiculos',
  templateUrl: './seguros-vehiculos.page.html',
  styleUrls: ['./seguros-vehiculos.page.scss'],
})
export class SegurosVehiculosPage implements OnInit {
  input: {
    codigo?: string;
    Placa?: string;
    aseguradoras?: string;
  ...
  paraeliminar(){
    ...
    this.authService.ApiSendData(this.Constants._APIregistrarsolicitudVehiculos, parameliminar).subscribe(data => {
      if(data[0][0].Codigo=="401"){
        this.NitsServices.mostrarMensajes(data)
        localStorage.clear()
        this.NitsServices.LogIn()
    ...
    if (!this.edit) {
        ...
        this.buscardatos()
          ...
          if (this.quees == 'edita' && this.select['modificados'].length >0) {
           ...
           if(this.input['Placa']==undefined || this.input['Placa']=='' || this.input['aseguradoras']==undefined
            ||this.input['Beneficiario']==undefined || this.input['Beneficiario']==''){
           ...
           this.authService.ApiSendData(this.Constants._APIregistrarsolicitudVehiculos, paramsnuevo).subscribe(data => {
            if(data[0][0].Codigo=="401"){
              this.NitsServices.mostrarMensajes(data)
              localStorage.clear()
              this.NitsServices.LogIn()
  ...
  elimina(beneficiarios){
    ...
    this.authService.ApiSendData(this.Constants._APIregistrarsolicitudVehiculos, paraeliminar).subscribe(data => {
      if(data[0][0].Codigo=="401"){
        this.NitsServices.mostrarMensajes(data)
        localStorage.clear()
        this.NitsServices.LogIn()
