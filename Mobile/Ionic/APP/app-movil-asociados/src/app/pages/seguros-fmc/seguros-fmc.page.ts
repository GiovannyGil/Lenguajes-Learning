...
@Component({
  selector: 'app-seguros-fmc',
  templateUrl: './seguros-fmc.page.html',
  styleUrls: ['./seguros-fmc.page.scss'],
})
export class SegurosFmcPage implements OnInit {
  input: {
    codigo?: string;
  ...
  paraeliminar(){
    ...
    this.authService.ApiSendData(this.Constants._APIregistrarsolicitudFMC, parameliminar).subscribe(data => {
      if(data[0][0].Codigo=="401"){
        this.NitsServices.mostrarMensajes(data)
        localStorage.clear()
        this.NitsServices.LogIn()
    ...
    if (!this.edit) {
        ...
        this.buscardatos()
          ...
          if (this.quees == 'edita' && this.select['modificados'].length >0) {
           ...
           if(this.input['FechaNacimiento']==undefined || this.input['parentesco']==undefined
            ||this.input['DocumentoBeneficiario']==undefined || this.input['DocumentoBeneficiario']=='' ||this.input['nombreBeneficiario']==undefined || this.input['nombreBeneficiario']==''){
              this.NitsServices.AlertMsm('Debe completar todos los campos')
              return
            }
            if(this.input['FechaNacimiento']!=undefined){
              this.input['FechaNacimiento']=moment(this.input['FechaNacimiento']).format('YYYYMMDD')
              }
           ...
           this.authService.ApiSendData(this.Constants._APIregistrarsolicitudFMC, paramsnuevo).subscribe(data => {
            if(data[0][0].Codigo=="401"){
              this.NitsServices.mostrarMensajes(data)
              localStorage.clear()
              this.NitsServices.LogIn()
  ...
  elimina(beneficiarios){
    ...
    this.authService.ApiSendData(this.Constants._APIregistrarsolicitudFMC, paraeliminar).subscribe(data => {
      if(data[0][0].Codigo=="401"){
        this.NitsServices.mostrarMensajes(data)
        localStorage.clear()
        this.NitsServices.LogIn()
