...
@Component({
  selector: 'app-seguros-exequial',
  templateUrl: './seguros-exequial.page.html',
  styleUrls: ['./seguros-exequial.page.scss'],
})
export class SegurosExequialPage implements OnInit {
  input: {
    codigo?: string;
    nombre?: string;
    parentesco?: string;
  ...
  paraeliminar(){
    ...
    this.authService.ApiSendData(this.Constants._APIregistrarsolicitudexequial, parameliminar).subscribe(data => {
      if(data[0][0].Codigo=="401"){
        this.NitsServices.mostrarMensajes(data)
        localStorage.clear()
        this.NitsServices.LogIn()
        ...
        this.buscardatos()
          ...
          if (this.quees == 'edita' && this.select['modificados'].length >0) {
           ...
           if(this.input['nombre']==undefined || this.input['nombre']=='' || this.input['parentesco']==undefined || this.input['parentesco']==''
            ||this.input['empresa']==undefined || this.input['empresa']==''||this.input['cedulabeneficiario']==undefined || this.input['cedulabeneficiario']==''){
           ...
           this.authService.ApiSendData(this.Constants._APIregistrarsolicitudexequial, paramsnuevo).subscribe(data => {
            if(data[0][0].Codigo=="401"){
              this.NitsServices.mostrarMensajes(data)
              localStorage.clear()
              this.NitsServices.LogIn()
  ...
  elimina(beneficiarios){
    ...
    this.authService.ApiSendData(this.Constants._APIregistrarsolicitudexequial, paraeliminar).subscribe(data => {
      if(data[0][0].Codigo=="401"){
        this.NitsServices.mostrarMensajes(data)
        localStorage.clear()
        this.NitsServices.LogIn()
