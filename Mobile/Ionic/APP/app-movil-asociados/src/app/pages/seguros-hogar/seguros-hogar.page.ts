...
@Component({
  selector: 'app-seguros-hogar',
  templateUrl: './seguros-hogar.page.html',
  styleUrls: ['./seguros-hogar.page.scss'],
})
export class SegurosHogarPage implements OnInit {
  input: {
    codigo?: string;
    Direccion?: string;
    Ciudad?: string;
  ...
  paraeliminar(){
    ...
    this.authService.ApiSendData(this.Constants._APIregistrarsolicitudHogar, parameliminar).subscribe(data => {
      if(data[0][0].Codigo=="401"){
        this.NitsServices.mostrarMensajes(data)
        localStorage.clear()
        this.NitsServices.LogIn()
    ...
    if (!this.edit) {
        ...
        this.buscardatos()
          ...
          if (this.quees == 'edita' && this.select['modificados'].length >0) {
           ...
           if(this.input['Direccion']==undefined || this.input['Direccion']=='' || this.input['Ciudad']==undefined || this.input['Ciudad']==''
            ...
            ||this.input['valor3']==undefined || this.input['valor3']==''){
           ...
           this.authService.ApiSendData(this.Constants._APIregistrarsolicitudHogar, paramsnuevo).subscribe(data => {
            if(data[0][0].Codigo=="401"){
              this.NitsServices.mostrarMensajes(data)
              localStorage.clear()
              this.NitsServices.LogIn()
  ...
  elimina(beneficiarios){
    ...
    this.authService.ApiSendData(this.Constants._APIregistrarsolicitudHogar, paraeliminar).subscribe(data => {
      if(data[0][0].Codigo=="401"){
        this.NitsServices.mostrarMensajes(data)
        localStorage.clear()
        this.NitsServices.LogIn()
