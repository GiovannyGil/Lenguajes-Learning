...
@Component({
  selector: 'app-preseguimiento-creditos',
  templateUrl: './preseguimiento-creditos.page.html',
  styleUrls: ['./preseguimiento-creditos.page.scss'],
})
export class PreseguimientoCreditosPage implements OnInit {
  seguimientos: any
  credito: any
  params: any
  dateFrom: any
  ...
  constructor(
    ...
    this.mostrardetalle = localStorage.getItem("detalleCre")
    this.seguimientos = this.router.getCurrentNavigation().extras.queryParams.pseguimiento
    this.credito = this.router.getCurrentNavigation().extras.queryParams.pcredito
  ...
  ngOnInit() {
    ...
    if (mesesCre == 0) {
      ...
      this.fechainicio.setMonth(this.fechainicio.getMonth() - mesesCre);
    ...
    this.authService.ApiSendData(this.Constants._APIseguimientoCreditos, this.params).subscribe(data => {
      if (data[0].length > 0) {
  ...
  Detalle(data) {
    if (this.mostrardetalle == 'S') {
      ...
      this.router.navigateByUrl('/detalle-creditos', navigationExtras)
    }
  }
