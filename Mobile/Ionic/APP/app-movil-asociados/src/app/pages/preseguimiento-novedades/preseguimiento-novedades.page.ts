...
@Component({
  selector: 'app-preseguimiento-novedades',
  templateUrl: './preseguimiento-novedades.page.html',
  styleUrls: ['./preseguimiento-novedades.page.scss'],
})
export class PreseguimientoNovedadesPage implements OnInit {
  seguimientos:any
  novedad: any
  params: any
  dateFrom: any
  ...
  constructor(
    ...
    this.mostrardetalle=localStorage.getItem("detalleNov")
    this.seguimientos=this.router.getCurrentNavigation().extras.queryParams.pseguimiento
    this.novedad=this.router.getCurrentNavigation().extras.queryParams.pnovedad
  ...
  ngOnInit() {
    ...
    if (mesesNov == 0) {
      ...
      this.fechainicio.setMonth(this.fechainicio.getMonth() - mesesNov);
  ...
  this.authService.ApiSendData(this.Constants._APIseguimientoNovedades, this.params).subscribe(data => {
  if(data[0].length>0){
...
Detalle(data){
  if(this.mostrardetalle=='S') {
    ...
    this.router.navigateByUrl('/detalle-novedades', navigationExtras)
  }
}
